...
string read_next_json_token(istream &in) {
	...
	if(cget=='\"') {
		...
		//cerr << "returning token |" << total_string << "|" << endl;
		return total_string;
	}
	//single character tokens
	if(single_char_tokens.find(cget)!=string::npos) {
		//cerr << "returning token |" << string(1,cget) << "|" << endl;
...
istream &operator>>(istream &in,JSONObject &obj) {
	...
	while((pairsep.size()!=0)&&(pairsep==",")) {
		...
		if(sep[0]!=':') cerr << "JSON warning, separator wasn't a colon, it was " << sep << endl;
		JSONValue val;
		in >> val;
		obj[key]=val;
...
JSONValue::JSONValue(const long long &i) {
	data=NULL;
	setIntValue(i);
}
...
void JSONValue::setIntValue(long long i) {
	delete_data();
	type=INTEGER;
	data=new long;
	*((long long*)data)=i;
...
const long long &JSONValue::getIntValue() const {
#ifdef JSON_TYPE_ERRORS_ON
	if(getType()!=INTEGER)
...
long long &JSONValue::getIntValue() {
#ifdef JSON_TYPE_ERRORS_ON
	if(getType()!=INTEGER)
	{
	cerr << "Error requesting JSON int value from " << getTypeAsString() << endl;
